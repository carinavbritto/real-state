...
const Card: React.FC<CardProps> = ({ title, subtitle, className }) => {
  return (
    ...
    >
      {title && (
        <h2 className="sm:text-2xl md:text-3xl 2xl:text-4xl font-bold tracking-tight leading-tight text-white-0">
          {title}
        </h2>
      )}
      {subtitle && (
        <p className="text-gray-600 sm:text-sm md:text-base 2xl:text-lg 2xl:leading-6 font-medium text-gray-60">
          {subtitle}
        </p>
      )}
    </div>
